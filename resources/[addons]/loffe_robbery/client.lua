...
function HashInTable(hash)
    for k, v in pairs(GunAutorized) do 
        if (hash == v) then 
            return true 
        end 
    end 
...
AddEventHandler('loffe_robbery:robberyOver', function(playerId)
    robbing = false
end)
...
AddEventHandler('loffe_robbery:rob', function(i, playerId)
                    ...
                    while true do
                        ...
                        if DoesEntityExist(bag) then
                            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(bag), true) <= 1.5 then
                                ...
                                TriggerServerEvent('loffe_robbery:pickUp', i, playerId, bag)
                                DeleteObject(bag)
                                break
                            end
                        else
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        local hash = GetSelectedPedWeapon(me)
        if GetCurrentPedWeapon(me, GetHashKey('WEAPON_UNARMED')) then
                    ...
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 5.0 then
                        if not robbing then
                            ...
                            if canRob == true then
                                ...
                                TriggerEvent('pogressBar:drawBar', 20000, 'Braquage en cours...')
                                ...
                                TriggerServerEvent('loffe_robbery:kodaaa', i)
                                local scared = 0
                                ...
                                if GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 7.5 then
                                    if not IsPedDeadOrDying(peds[i]) then
                                        TriggerServerEvent('loffe_robbery:rob', i, GetPlayerServerId(PlayerId()))
                                        while robbing do Wait(0) if IsPedDeadOrDying(peds[i]) then robbing = false end end
                                    end
                                else
                                    ClearPedTasks(peds[i])

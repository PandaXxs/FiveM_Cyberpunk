...
RegisterServerEvent('irp_discord:removeFromQueue')
AddEventHandler('irp_discord:removeFromQueue', function()
...
lib.callback.register('irp_discord:getConfig', function(source)
    local cConfig = copyTable(Config)
    cConfig.DiscordInfo = nil 
    return cConfig
end)

lib.callback.register('irp_discord:checkForRole', function(source, role)
    local hasRole = checkRole(source, role)
    return hasRole
end)

lib.callback.register('irp_discord:getRoles', function(source, role)

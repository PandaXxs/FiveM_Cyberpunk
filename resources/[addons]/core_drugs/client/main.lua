...
Citizen.CreateThread(
    function()
        while true do
            if strength then
                N_0x4757f00bc6323cfe(GetHashKey("WEAPON_UNARMED"), 2.7) 
                Citizen.Wait(5)
            else
...
function addEffect(effect, status)
        ...
        if status then
            Citizen.CreateThread(
                function()
                    ...
                    SetEntityMaxSpeed(PlayerPedId(), 12.0)
            ...
            TriggerServerEvent('projectrpUtils:sendAnnounce', 'Contre coup (Synthécoke)', 'Vous êtes épuisé.', 'center-left', '#520000', '#909296', "fas fa-info", '#909296', 6000, GetPlayerServerId(PlayerId()))
        end
    ...
    elseif effect == "noSprint" then
        if status then
            Citizen.CreateThread(
                function()
                    SetEntityMaxSpeed(PlayerPedId(), 3.0)
    ...
    elseif effect == "moreStrength" then
        if status then
            ...
            TriggerServerEvent('projectrpUtils:sendAnnounce', 'Contre coup (Ampoule de smash)', 'Vous êtes affamées.', 'center-left', '#520000', '#909296', "fas fa-info", '#909296', 6000, GetPlayerServerId(PlayerId()))
            strength = false
        end
    elseif effect == "malusSmash" then
        if status then
            Citizen.CreateThread(
                function()
                    TriggerEvent('esx_status:remove', 'hunger', 500000)
                    TriggerEvent('esx_status:remove', 'thirst', 500000)
                end
            )
        end
    elseif effect == "addiction" then
        if status then
            Citizen.CreateThread(
                function()
                    TriggerEvent("esx_status:add", "addiction", math.random(15000, 70000))
                end
            )
        end
    ...
    elseif effect == "armor" then
        if status then
            ...
            SetPedArmour(ped, 0)
            TriggerServerEvent('projectrpUtils:sendAnnounce', 'Contre coup (Pillule de dorph)', 'Vous ne vous sentez pas bien allez vous faire soigné.', 'center-left', '#520000', '#909296', "fas fa-info", '#909296', 6000, GetPlayerServerId(PlayerId()))
        end
    ...
    Citizen.CreateThread(
        function()
            ...
            for _, effect in ipairs(info.Effects) do
                for _, subeffect in ipairs(effect.Effect) do
                   addEffect(subeffect, true)
                   print('effet')
                end

                Citizen.Wait(6000 + (effect.Time * 1000))

                for _, subeffect in ipairs(effect.Effect) do
                    addEffect(subeffect, false)
                    print('pus d\'effet')
                end
            end

            Citizen.Wait(6000 + (info.Time * 1000))
            ...
            if not Initialized then
                ...
                if nProcess ~= false then
                    ...
                    DrawText3D(
                        ...
                        tableCoords[2],
                        tableCoords[3] + 1.0,
                        Config.Text["processing_table_holo"]
                    )

...
CreateThread(function()
	while true do
		if nuiReady and ESX.PlayerLoaded then
			...
			if cfg.drugAddiction then
				TriggerEvent('esx_status:getStatus', 'addiction', function(status)
					if status.val > 0 then
						SendMessage('setDrugAddiction', {
							current = status.val,
							max = 100000
						})
						isAddicted = false
					elseif not isAddicted then
						SendMessage('setDrugAddiction', false)
						isAddicted = true
					end
				end)
			end
		...
		if nuiReady and ESX.PlayerLoaded then
			repeat
				...
				if cfg.drugAddiction then
					TriggerEvent('esx_status:getStatus', 'addiction', function(status)
						if status then 
							drugAddiction = status.val / 100000
						end
					end)
				end
				Wait(100)
			...
			SendMessage('status', {
				...
				drugAddiction = cfg.drugAddiction and drugAddiction
			})
		end
...
RegisterNetEvent('dx_hud:updateDrugStatus')
AddEventHandler('dx_hud:updateDrugStatus', function(drug)
	drug = drug
end)
...
window.onload = event => {
  ...
  const DrugAddiction = document.getElementById('DrugAddictionIndicator');
  ...
  const DrugAddictionIcon = document.getElementById('DrugAddictionIcon');
  ...
  window.addEventListener('message', function (event) {
    ...
    if(action == 'setDrugAddiction') {
      if (data) {
        DrugAddiction.style.display = 'block';
        let drugAddiction = data.current / data.max;
        drugAddiction < 0 && (drugAddiction = 0);
        drugAddiction < 0.1 && DrugAddictionIcon.classList.toggle('flash');

        Circle.DrugAddictionIndicator.path.setAttribute('stroke', drugAddiction < 0.1 ? 'rgb(255, 0, 0)' : 'rgb(79, 236, 254)');
        Circle.DrugAddictionIndicator.animate(drugAddiction);
      } else {
        Circle.DrugAddictionIndicator.animate(1, function () {
          DrugAddiction.style.display = 'none';
        });
      }
    }

...
CreateThread(function()
    ...
    if Config.targetSystem then
        ...
        TriggerEvent('wasabi_burgersandy:addTarget', data)
    end
end)
...
CreateThread(function()
    ...
    for k,v in pairs(Config.Locations) do
                    ...
                    if ESX.PlayerData.job.name == 'burgersandy' then
                        ...
                        if dist <= v.Coffre.Distance then
                            ...
                            if IsControlJustReleased(0, 38) then
                                exports.ox_inventory:openInventory('stash', {id = 'society_burgersandy', owner = 'society_burgersandy'})
                            end
                        else
                            if textUI then
                                lib.hideTextUI()
        ...
        if v.BurgerSandySupplies.Enabled then
            if Config.targetSystem then
                local data = {
                    targetType = 'AddBoxZone',
                    identifier = k..'_BurgerSandysup',
                    coords = v.BurgerSandySupplies.Coords,
                    heading = v.BurgerSandySupplies.Heading,
                    width = 1.0,
                    length = 1.0,
                    minZ = v.BurgerSandySupplies.Coords.z-1.5,
                    maxZ = v.BurgerSandySupplies.Coords.z+1.5,
                    distance = 1.5,
                    options = {
                        {
                            event = 'wasabi_burgersandy:BurgerSandySuppliesMenu',
                            icon = 'fas fa-toolbox',
            ...
            CreateThread(function() 
                ...
                while true do
                    ...
                    local dist = #(vector3(coords.x, coords.y, coords.z) - vector3(v.BurgerSandySupplies.Coords.x, v.BurgerSandySupplies.Coords.y, v.BurgerSandySupplies.Coords.z))
                    if dist <= 30 and not pedSpawned then
                        lib.requestAnimDict('mini@strip_club@idles@bouncer@base', 100)
                        lib.requestModel(v.BurgerSandySupplies.Ped, 100)
                        ped = CreatePed(28, v.BurgerSandySupplies.Ped, v.BurgerSandySupplies.Coords.x, v.BurgerSandySupplies.Coords.y, v.BurgerSandySupplies.Coords.z, v.BurgerSandySupplies.Heading, false, false)
        ...
        if v.BossMenu.Enabled then
            if v.BossMenu?.Target?.enabled then
                local data = {
                    ...
                    options = {
                        {
                            event = 'wasabi_burgersandy:openBossMenu',
                    ...
                    while true do
                        ...
                        if ESX.PlayerData.job.name == 'burgersandy' then
                            ...
                            if dist <= v.BossMenu.Distance then
                                ...
                                sleep = 0
                                if IsControlJustReleased(0, 38) then
                                    TriggerEvent('esx_society:openBossMenu', 'burgersandy', function(data, menu)
                                        menu.close()
                                    end, {wash = false})
                                end
                            else
...
AddEventHandler('wasabi_burgersandy:buyItem', function(data)
    TriggerServerEvent('wasabi_burgersandy:restock', data)
end)

AddEventHandler('wasabi_burgersandy:openBossMenu', function()
	TriggerEvent('esx_society:openBossMenu', 'burgersandy', function(data, menu)
		menu.close()
	end, {wash = false})
end)

AddEventHandler('wasabi_burgersandy:spawnVehicle', function(data)
    inMenu = false
    local model = data.model
    local category = Config.Locations[data.BurgerSandy].Vehicles.Options[data.model].category
    local spawnLoc = Config.Locations[data.BurgerSandy].Vehicles.Spawn[category]
...
AddEventHandler('wasabi_burgersandy:billCustomer', function(data)
    if ESX.PlayerData.job.name == 'burgersandy' then
        local player, dist = ESX.Game.GetClosestPlayer()
        if player == -1 or dist > 4.0 then
            TriggerEvent('wasabi_burgersandy:notify', Strings.no_nearby, Strings.no_nearby_desc, 'error')
        else
            ...
            if amount < 1 then
                TriggerEvent('wasabi_burgersandy:notify', Strings.invalid_entry, Strings.invalid_entry_desc, 'error')
            elseif Config.billingSystem == 'okok' then
                local data =  {
                    target = targetId,
                    invoice_value = amount,
                    invoice_item = Strings.BurgerSandy_services,
                    society = 'society_BurgerSandy',
                    society_name = 'burgersandy',
                    invoice_notes = ''
                }
                TriggerServerEvent('okokBilling:CreateInvoice', data)
            elseif Config.billingSystem == 'pefcl' then
                TriggerServerEvent('wasabi_burgersandy:billPlayer', targetId, amount)
            else
                TriggerServerEvent('esx_billing:sendBill', targetId, 'society_BurgerSandy', 'burgersandy', amount)
            end
        end
    end
end)

AddEventHandler('wasabi_burgersandy:BurgerSandySuppliesMenu', function(data)
    BurgerSandySuppliesMenu(data.BurgerSandy)
end)

AddEventHandler('wasabi_burgersandy:gItem', function(data)
    gItem(data)
end)

RegisterCommand('burgerSandyJobMenu', function()
    openJobMenu()
end)

AddEventHandler('wasabi_burgersandy:burgerSandyJobMenu', function()
    openJobMenu()
end)

AddEventHandler('wasabi_burgersandy:announceMenu', function()
    openAnnounceMenu()
end)

AddEventHandler('wasabi_burgersandy:announce', function(data)
    if data.type == 'custom' then
        local input = lib.inputDialog('Annonce personalisée', {'Message'})
        if not input then return end
        TriggerServerEvent('wasabi_burgersandy:announce', data.type, input[1])
        return
    end
    TriggerServerEvent('wasabi_burgersandy:announce', data.type)
end)

AddEventHandler('wasabi_burgersandy:menu', function(data)
	if lib.progressCircle({
        ...
        TriggerServerEvent('wasabi_burgersandy:giveitem', data.menu)
    end
end)

AddEventHandler('wasabi_burgersandy:boisson', function(data)
	if lib.progressCircle({
		duration = 1000,
		position = 'bottom',
        ...
        TriggerServerEvent('wasabi_burgersandy:giveitem', data.item)
    end
end)

AddEventHandler('wasabi_burgersandy:dessert', function(data)
	if lib.progressCircle({
		duration = 1000,
		position = 'bottom',
...
TriggerEvent('chat:removeSuggestion', '/burgerSandyJobMenu')

RegisterKeyMapping('burgerSandyJobMenu', Strings.key_map_text, 'keyboard', Config.jobMenu)

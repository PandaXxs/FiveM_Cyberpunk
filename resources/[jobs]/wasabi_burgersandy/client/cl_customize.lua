...
RegisterNetEvent('wasabi_burgersandy:notify', function(title, desc, style, icon) -- Edit notifications here with custom notification system
    if icon then
        lib.notify({
            title = title,
            description = desc,
...
-- Add target event
AddEventHandler('wasabi_burgersandy:addTarget', function(d)
...
-- Remove target event
AddEventHandler('wasabi_burgersandy:removeTarget', function(identifier)
    exports.qtarget:RemoveZone(identifier)
end)

AddEventHandler('wasabi_burgersandy:changeClothes', function(data) -- Change with your own code here if you want?

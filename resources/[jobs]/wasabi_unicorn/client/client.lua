...
CreateThread(function()
    ...
    if Config.targetSystem then
        ...
        TriggerEvent('wasabi_unipunk:addTarget', data)
    end
end)
...
AddEventHandler('esx:playerLoaded', function(xPlayer)
    ...
    if ESX.PlayerData.job.name == 'unipunk' then
        TriggerServerEvent('wasabi_unipunk:requestSync')
    end
end)
...
CreateThread(function()
    ...
    for k,v in pairs(Config.Locations) do
        ...
        if v.BossMenu.Enabled then
            if v.BossMenu?.Target?.enabled then
                local data = {
                    ...
                    options = {
                        {
                            event = 'wasabi_unipunk:openBossMenu',
                        ...
                        if ESX.PlayerData.job.name == 'unipunk' then
                            ...
                            if dist <= v.BossMenu.Distance then
                                ...
                                if IsControlJustReleased(0, 38) then
                                    TriggerEvent('esx_society:openBossMenu', 'unipunk', function(data, menu)
                                        menu.close()
                                    end, {wash = false})
                                end
                            else
                ...
                while true do
                    ...
                    if ESX.PlayerData.job.name == 'unipunk' then
                        ...
                        if dist <= v.Coffre.Distance then
                            ...
                            sleep = 0
                            if IsControlJustReleased(0, 38) then
                                exports.ox_inventory:openInventory('stash', {id = 'society_unipunk', owner = 'society_unipunk'})
                            end
                        else
                            if textUI then
                                lib.hideTextUI()
        ...
        if v.unipunkSupplies.Enabled then
            if Config.targetSystem then
                local data = {
                    targetType = 'AddBoxZone',
                    identifier = k..'_unipunksup',
                    coords = v.unipunkSupplies.Coords,
                    heading = v.unipunkSupplies.Heading,
                    width = 1.0,
                    length = 1.0,
                    minZ = v.unipunkSupplies.Coords.z-1.5,
                    maxZ = v.unipunkSupplies.Coords.z+1.5,
                    distance = 1.5,
                    options = {
                        {
                            event = 'wasabi_unipunk:unipunkSuppliesMenu',
                            icon = 'fas fa-toolbox',
            ...
            CreateThread(function() 
                ...
                while true do
                    ...
                    local dist = #(vector3(coords.x, coords.y, coords.z) - vector3(v.unipunkSupplies.Coords.x, v.unipunkSupplies.Coords.y, v.unipunkSupplies.Coords.z))
                    if dist <= 30 and not pedSpawned then
                        lib.requestAnimDict('mini@strip_club@idles@bouncer@base', 100)
                        lib.requestModel(v.unipunkSupplies.Ped, 100)
                        ped = CreatePed(28, v.unipunkSupplies.Ped, v.unipunkSupplies.Coords.x, v.unipunkSupplies.Coords.y, v.unipunkSupplies.Coords.z, v.unipunkSupplies.Heading, false, false)
...
AddEventHandler('wasabi_unipunk:buyItem', function(data)
    TriggerServerEvent('wasabi_unipunk:restock', data)
end)

AddEventHandler('wasabi_unipunk:openBossMenu', function()
	TriggerEvent('esx_society:openBossMenu', 'unipunk', function(data, menu)
		menu.close()
	end, {wash = false})
end)

AddEventHandler('wasabi_unipunk:spawnVehicle', function(data)
    inMenu = false
    local model = data.model
    local category = Config.Locations[data.unipunk].Vehicles.Options[data.model].category
    local spawnLoc = Config.Locations[data.unipunk].Vehicles.Spawn[category]
...
AddEventHandler('wasabi_unipunk:billCustomer', function()
    if ESX.PlayerData.job.name == 'unipunk' then
        local player, dist = ESX.Game.GetClosestPlayer()
        if player == -1 or dist > 4.0 then
            TriggerEvent('wasabi_unipunk:notify', Strings.no_nearby, Strings.no_nearby_desc, 'error')
        else
            ...
            if amount < 1 then
                TriggerEvent('wasabi_unipunk:notify', Strings.invalid_entry, Strings.invalid_entry_desc, 'error')
            elseif Config.billingSystem == 'okok' then
                local data =  {
                    target = targetId,
                    invoice_value = amount,
                    invoice_item = Strings.unipunk_services,
                    society = 'society_unipunk',
                    society_name = 'unipunk',
                    invoice_notes = ''
                }
                TriggerServerEvent('okokBilling:CreateInvoice', data)
            elseif Config.billingSystem == 'pefcl' then
                TriggerServerEvent('wasabi_unipunk:billPlayer', targetId, amount)
            else
                TriggerServerEvent('esx_billing:sendBill', targetId, 'society_unipunk', 'unipunk', amount)
            end
        end
    end
end)

AddEventHandler('wasabi_unipunk:unipunkSuppliesMenu', function(data)
    unipunkSuppliesMenu(data.unipunk)
end)

AddEventHandler('wasabi_unipunk:gItem', function(data)
    gItem(data)
end)

RegisterCommand('unipunkJobMenu', function()
    openJobMenu()
end)

AddEventHandler('wasabi_unipunk:unipunkJobMenu', function()
    openJobMenu()
end)

AddEventHandler('wasabi_unipunk:announceMenu', function()
    openAnnounceMenu()
end)

AddEventHandler('wasabi_unipunk:announce', function(data)
    if data.type == 'custom' then
        local input = lib.inputDialog('Annonce personalisée', {'Message'})
        if not input then return end
        TriggerServerEvent('wasabi_unipunk:announce', data.type, input[1])
        return
    end
    TriggerServerEvent('wasabi_unipunk:announce', data.type)
end)

TriggerEvent('chat:removeSuggestion', '/unipunkJobMenu')

RegisterKeyMapping('unipunkJobMenu', Strings.key_map_text, 'keyboard', Config.jobMenu)

...
RegisterNetEvent('wasabi_unipunk:notify', function(title, desc, style, icon) -- Edit notifications here with custom notification system
    if icon then
        lib.notify({
            title = title,
            description = desc,
...
-- Add target event
AddEventHandler('wasabi_unipunk:addTarget', function(d)
...
-- Remove target event
AddEventHandler('wasabi_unipunk:removeTarget', function(identifier)
    exports.qtarget:RemoveZone(identifier)
end)

AddEventHandler('wasabi_unipunk:changeClothes', function(data) -- Change with your own code here if you want?

...
ESX.RegisterServerCallback('esx_vehicleshop:buyVehicle', function(source, cb, model, plate)
	...
	if tonumber(modelPrice) and xPlayer.getMoney() >= tonumber(modelPrice) then
...
-- Commande de give de véhicule (pour les admins) ESX
RegisterCommand('givecaradmin', function(source, args, rawCommand)
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getGroup() == 'admin' or xPlayer.getGroup() == 'superadmin' then
		if args[1] and args[2] then
			...
			-- random plate generation format -> ABC 123

			-- Random PlateLetters max 3
			local plateLetters = {}
			for i = 65, 90 do
				table.insert(plateLetters, string.char(i))
			end

			-- Random PlateNumbers max 3

			local plate = string.upper(plateLetters[math.random(1, #plateLetters)] .. plateLetters[math.random(1, #plateLetters)] .. plateLetters[math.random(1, #plateLetters)] .. ' '  .. math.random(100, 999))
			...
			MySQL.Async.fetchAll('SELECT plate FROM owned_vehicles WHERE plate = @plate', {
				...
				if result[1] ~= nil then
					TriggerClientEvent('esx:showNotification', xPlayer.source, 'La plaque est déjà utilisée')
					return
				end
			end)
			...
			local vehicle = CreateVehicle(vehicle, targetcoords, 0.0, true, false)
			SetVehicleNumberPlateText(vehicle, plate)
			SetPedIntoVehicle(target, vehicle, -1)

			Citizen.Wait(2000)

			-- check if vehicle spawn in map
			if not DoesEntityExist(vehicle) then
				TriggerClientEvent('esx:showNotification', xPlayer.source, 'Ce véhicule n\'existe pas')
				return
			end
			
			MySQL.insert('INSERT INTO owned_vehicles (owner, plate, vehicle) VALUES (?, ?, ?)', {xPlayer.identifier, plate, json.encode({model = joaat(args[2]), plate = plate})
			}, function(rowsChanged)
				TriggerClientEvent('esx:showNotification', xPlayer.source, 'Vous avez donné un véhicule: ' .. vehicleName .. '(Plaque: ' .. plate .. ')')
				TriggerClientEvent('esx:showNotification', target, 'Vous avez reçu un véhicule: ' .. vehicleName .. '(Plaque: ' .. plate .. ')')
				exports.wasabi_carlock:GiveKeys(plate, target)
			end)


		else
			print(('esx_vehicleshop: %s attempted to exploit the garage!'):format(xPlayer.identifier))
		end
	else
		print(('esx_vehicleshop: %s attempted to exploit the garage!'):format(xPlayer.identifier))
	end

local DISCORD_NAME = "Death Logs"
local DISCORD_IMAGE = "https://i.imgur.com/HMP7cbj.png"
...
function LogPlayerInfo(src)
    local playerOnline = GetPlayer(src)
...
local function sendToDiscord(victim, cause, killer)
	if not Config.DeathLogs then return end
	if SConfig.webhook == "CHANGE_ME" or SConfig.webhook == "" then
    ...
    if killer then
        ...
        PerformHttpRequest(SConfig.webhook, function() end, 'POST', json.encode({username = DISCORD_NAME, embeds = connect, avatarrl = DISCORD_IMAGE}), { ['Content-Type'] = 'application/json' })
    else
        connect = {
			{
				["color"] = SConfig.color,
...
RegisterServerEvent('wasabi_ambulance:log')
AddEventHandler('wasabi_ambulance:log', function(cause, killer)
    sendToDiscord(source, cause, killer)
end)
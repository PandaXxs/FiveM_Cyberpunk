...
CreateThread(function()
    ...
    if Config.targetSystem then
        local data = {
            targetType = 'Model',
            models = {`xm_prop_x17_bag_med_01a`},
            options = {
                {
...
RegisterNetEvent('wasabi_ambulance:playerLoaded')
AddEventHandler('wasabi_ambulance:playerLoaded', function()
    local ped = PlayerPedId() -- slower PCs do not seem to be caching ped in ox lib fast enough
    local player = PlayerId()
    Wait(5500) -- Wait for slow clients
    SetEntityMaxHealth(ped, 200)
    SetEntityHealth(ped, 200)
    SetPlayerHealthRechargeMultiplier(player, 0.0)
    SetPlayerHealthRechargeLimit(player, 0.0)
    if Config.AntiCombatLog.enabled then
        local dead = lib.callback.await('wasabi_ambulance:checkDeath', 100)
        if dead then
...
AddEventHandler('wasabi_ambulance:onPlayerSpawn', function()
    isDead = false
    local ped = PlayerPedId() -- Players seem to be not loading cache.ped fast enough
    SetEntityMaxHealth(ped, 200)
    SetEntityHealth(ped, 200)
    if firstSpawn then
        firstSpawn = false
...
AddEventHandler('wasabi_ambulance:onPlayerDeath', function(data)
    ...
    if Config.DeathLogs then
        ...
        if deathCause == 'suicide' or deathCause == 'unknown' then
            TriggerServerEvent('wasabi_ambulance:log', dCause, nil)
        else
            TriggerServerEvent('wasabi_ambulance:log', dCause, GetPlayerServerId(killer))
        end
    end
    OnPlayerDeath()
end)
...
--For qbcore
RegisterNetEvent('wasabi_ambulance:killPlayer')
AddEventHandler('wasabi_ambulance:killPlayer', function()
...
CreateThread(function()
    ...
    for k,v in pairs(Config.Locations) do
        ...
        if v.MedicalWeapons.Enabled then
            CreateThread(function()
                local textUI
                while true do
                    local sleep = 1500
                    ...
                    local playerPed = cache.ped
                    local coords = GetEntityCoords(playerPed)
                    local dist = #(vector3(coords.x, coords.y, coords.z) - vector3(v.MedicalWeapons.Coords.x, v.MedicalWeapons.Coords.y, v.MedicalWeapons.Coords.z))
                    if dist <= 2 then
                        sleep = 0
                        if not textUI and hasJob then
                            ShowTextUI(Strings.open_weaponshop_ui)
                            textUI = true
                        end
                        if IsControlJustReleased(0, 38) and hasJob then
                            textUI = nil
                            HideTextUI()
                            medicalWeaponsMenu(k)
                        end
...
RegisterNetEvent('wasabi_ambulance:useBandage')
AddEventHandler('wasabi_ambulance:useBandage', function()
...
RegisterNetEvent('wasabi_ambulance:weaponRemove')
AddEventHandler('wasabi_ambulance:weaponRemove', function()
...
AddEventHandler('wasabi_ambulance:purchaseArmoury', function(data)
    if not HasGroup(Config.ambulanceJob) then return end
    local data = data
    data.quantity = 1
    if data.multiple then
        local input = lib.inputDialog('Quantité', {Strings.amount})
        if not input then return end
        local quantity = math.floor(tonumber(input[1]))
        if quantity < 1 then
            TriggerEvent('wasabi_ambulance:notify', Strings.invalid_amount, Strings.invalid_amount_desc, 'error')
        else
            data.quantity = quantity
        end
    end
    local canPurchase = lib.callback.await('wasabi_ambulance:canPurchase', 100, data)
    if canPurchase then
        TriggerEvent('wasabi_ambulance:notify', Strings.success, Strings.successful_purchase_desc, 'success')
    else
        TriggerEvent('wasabi_ambulance:notify', Strings.lacking_funds, Strings.lacking_funds_desc, 'error')
    end
end)
...
AddEventHandler('wasabi_ambulance:medicalWeaponsMenu', function(data)
    medicalWeaponsMenu(data.hospital)
end)

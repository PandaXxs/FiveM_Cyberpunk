...
RegisterNetEvent('wasabi_delamaintaxi:notify', function(title, desc, style, icon) -- Edit notifications here with custom notification system
    if icon then
        lib.notify({
            title = title,
            description = desc,
...
-- Add target event
AddEventHandler('wasabi_delamaintaxi:addTarget', function(d)
...
-- Remove target event
AddEventHandler('wasabi_delamaintaxi:removeTarget', function(identifier)
    exports.qtarget:RemoveZone(identifier)
end)

AddEventHandler('wasabi_delamaintaxi:changeClothes', function(data) -- Change with your own code here if you want?

...
CreateThread(function() 
    if Config.Zones then 
        for k, v in pairs(Config.Zones) do
            CreateBlip(v.coords, 356, 3, Strings.fishing_zone_blip, 0.70)
        end
    end
end)
...
AddEventHandler('wasabi_fishing:startFishing', function()
    -- boucle sur les zones de pêche
    if Config.Zones then 
        for k, v in pairs(Config.Zones) do
            -- si le joueur n'est pas dans une zone de pêche
            local dist = #(GetEntityCoords(cache.ped) - v.coords)
            if dist > v.radius then
                DrawMarker(1, v.coords.x, v.coords.y, v.coords.z, 0, 0, 0, 0, 0, 0, v.radius, v.radius, 0.5, 255, 255, 255, 100, 0, 0, 0, 0, 0, 0, 0)
                -- afficher une notification
                TriggerEvent('wasabi_fishing:notify', Strings.not_in_zone, Strings.not_in_zone_desc, 'error')
                return
            end
        end
    end

...
class Components{
	...
	static generateAllComponents(generatedData,generateType = "crypto"){
		$("#wrapper").empty()
		generatedData.forEach((form)=>{
			let objData = {}
			if(form.elementID.substring(1) == "transfer" && generateType == "crypto"){
				objData = new MultiComponentsForm(form)
			}else if(form.componentName == "trans" && generateType == "crypto"){
				objData = this.generateTransactionContainer(form)
			}
			else if(form.componentName == "moreGraph" && generateType == "crypto"){
				...
				if(generateType == "crypto"){
					if(form.componentName == "atmComponent"){
						return
					}
				}
...
class Render {
	...
	rendercryptoCard(){
		const cryptoData =  this.fullRenderData
		$("#cryptocard").empty();
		let template = `<div class="title">
		<h3>${cryptoData.cryptoName}</h3>
		<svg id="crypto-svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="11445.527 -3691.757 35 25">
			<defs>
			<style>
				.cls-1 {
				fill: orange;
	...
	<div class="content">
		...
		<div class="card-data">
			<p>${cryptoData.name}</p>
			<p>${cryptoData.createdDate}</p>
		</div>
		<p class="cardnumber">${cryptoData.cardNumber}</p>
		</div>`;
		$(template).appendTo("#cryptocard");
	}
		...
		<div id="money-containers">`;
			...
			template += `
				<div class="money-container">
					...
					${data.name == "cash" ? `<span id="money-${data.name}">${this.language.moneyFormat.replace("__replaceData__",data.amount)} $</span>` : `<span id="money-${data.name}">${this.language.moneyFormat.replace("__replaceData__",data.amount)}</span>`}
	...
	refresh(newData){
		switch (this.elementID) {
			case "your_money":
				...
				$("#money-crypto").html(`${this.language.moneyFormat.replace("__replaceData__",newData.crypto)}`);
				break;
			case "transaction": 
				this.fullRenderData = newData
...
$(document).ready(function(){
	...
	$.getJSON("config.json", function(data){
		...
		Utils.setTimeZone(lang == "FR" ? "fr-FR" : "fr-FR")
	...
	let cryptocard = new Render("cryptocard")
	let graph = ""
	var type = ""
	...
	window.addEventListener('message', ({data}) => {
		...
		if(data.showMenu){
			...
			const cryptoData = Utils.getCardData()
			datas.cryptoCardData.cardNumber = cryptoData.cardNumber
			datas.cryptoCardData.createdDate = cryptoData.cardExpiry
			cryptocard.setData(datas.cryptoCardData)
			cryptocard.rendercryptoCard()
			...
			updateTransData.unshift({time:Utils.dateFormat(new Date()),id: new Date().getTime(),amount: info.amount, type:info.label.toUpperCase(), balance: data.data.crypto})
			yourMoney.refresh(data.data)	
			transaction.refresh(updateTransData)
			if(type == "crypto") graph.refreshGraphData(updateTransData)
		}
	})
	...
	$(document).on('click','.accept-button',function(){
		...
		$.post('https://esx_crypto/clickButton', JSON.stringify(inputValues));
	...
	$(document).on('click','.exit-button',function(){
		...
		$.post('https://esx_crypto/close', JSON.stringify({}));
	})

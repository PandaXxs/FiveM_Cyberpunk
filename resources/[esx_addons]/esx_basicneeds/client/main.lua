...
AddEventHandler('esx_status:loaded', function(status)
	TriggerEvent('esx_status:registerStatus', 'hunger', 1000000, '#FFA500', function(status)
		return Config.Visible
	end, function(status)
		status.remove(75)
	end)

	TriggerEvent('esx_status:registerStatus', 'thirst', 1000000, '#00FFFF', function(status)
		return Config.Visible
	end, function(status)
		status.remove(50)
	end)
...
AddEventHandler('esx_status:onTick', function(data)
	...
	for k, v in pairs(data) do
		...
		elseif v.name == 'addiction' and v.val > 0 and not isAddicted then
			isAddicted = true
		elseif v.name == 'addiction' and v.val >= 100000 and isAddicted then

			TriggerEvent('projectrpUtils:notify', 'Vous faite une overdose', 'Aller vite vous faire soigner par un médecin.', 'inform')

			local dead = exports.wasabi_ambulance:isPlayerDead(GetPlayerServerId(PlayerId()))
			...
			elseif dead then
				isAddicted = false
				TriggerEvent('esx_status:set', 'addiction', 0)
			end
		elseif v.name == 'addiction' and v.val <= 5 and isAddicted then

			TriggerEvent('projectrpUtils:notify', 'Vous faite êtes en manque de substance', 'Aller vite vous faire soigner par un médecin.', 'inform')

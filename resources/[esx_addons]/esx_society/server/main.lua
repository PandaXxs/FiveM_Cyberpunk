...
AddEventHandler('esx_society:washMoney', function(society, amount)
	local source = source
	local xPlayer = ESX.GetPlayerFromId(source)
	local account = xPlayer.getAccount('crypto')
	amount = ESX.Math.Round(tonumber(amount))

	if xPlayer.job.name == society then
		if amount and amount > 0 and account.money >= amount then
			xPlayer.removeAccountMoney('crypto', amount, "Washing")
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, actionType)
	local xPlayer = ESX.GetPlayerFromId(source)
	local isBoss = xPlayer.job.grade_name == 'coboss' or xPlayer.job.grade_name == 'boss'
...
ESX.RegisterServerCallback('esx_society:setJobSalary', function(source, cb, job, grade, salary)
	...
	if xPlayer.job.name == job and xPlayer.job.grade_name == 'coboss' or xPlayer.job.grade_name == 'boss' then
...
function isPlayerBoss(playerId, job)
	...
	if xPlayer.job.name == job and xPlayer.job.grade_name == 'boss' then
		return true
	elseif xPlayer.job.name == job and xPlayer.job.grade_name == 'coboss' then
		return true
	else

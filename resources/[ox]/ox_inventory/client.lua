		...
		elseif invOpen ~= nil then
			if inv == 'policeevidence' then
				...
				elseif input % 1 ~= 0 then
					return lib.notify({ description = locale('locker_must_integer'), type = 'error' })
				elseif input > 5 or input < 1 then
					return lib.notify({ description = locale('locker_max_number'), type = 'error' })
				else
					data = input
				end
			end
